	...
	function tree_component () {
				...
				if(tmp.isdown) {
					...
					if(cnt.size() == 0) {
						...
						if(tmp.drag_help.children("IMG").size() == 0) {
							tmp.drag_help.append("<img style='position:absolute; " + (tmp.origin_tree.settings.ui.rtl ? "right" : "left" ) + ":4px; top:0px; background:white; padding:2px;' src='/esupport/javascript/themes/remove.png' />");
						}
						tmp.move_type	= false;
						tmp.ref_node	= false;
						tree_component.drag_drop.marker.hide();
		...
		return {
			...
			refresh : function (obj) {
				...
				if(this.settings.data.type == "xml_flat" || this.settings.data.type == "xml_nested") {
					...
					if(this.settings.data.json) {
						...
						$.post(this.settings.data.url,  function (data) {
								var str = "";
								var data = eval('(' + data + ')');
								if(data.length) {
			...
			parseJSON : function (data) {
				...
				if(data.attributes) {
					for(var i in data.attributes) {
						...
						/* Modified for JIRA ticket numbers B2C-61952/B2C-61115. This only workaround fix.Proper fix for this 
						 * issue should implement in droplet.As daily release i am fixing in this js, 
						 * In next mega release this code should remove and should implement in droplet
						*/
						else {
							str += " " + i + "='" + data.attributes[i] + "' ";
							if(data.attributes[i]=='9000848')
								str +="  style='width: 185px; border-bottom: 0px solid rgb(204, 204, 204); padding-bottom: 0px; margin-bottom: 0px;'";
						}
					}
				}
				if(data.state =="last" || data.state == "leaf");
				{
					str +="style ='float:none;'";
				}
			...
			open_branch : function (obj, disable_animation, callback) {
				...
				if (obj.hasClass("leaf") && !this.settings.selected==false) {
					// select a node if settings.selected has a value. (leaf nodes only...) 
					// Note: The TopicTreeDroplet does not send 'state' = 'open'/'closed' JSON attribute  
					// and this plugin assigns class="leaf" if there's no state attribute thereby preventing the node from opening/selecting     
					this.select_branch(obj,false);
				}			
				...
				if(this.settings.data.async && obj.find("li").size() == 0) {
					...
					if(this.settings.data.type == "xml_flat" || this.settings.data.type == "xml_nested") {
						...
						//alert(this.settings.data.url);
						/*	$.get(this.settings.data.url,  function (data, textStatus) {
								var data = eval('(' + data + ')');
