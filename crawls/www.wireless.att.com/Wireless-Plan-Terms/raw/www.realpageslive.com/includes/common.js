...
dirCode=null;if(selectedState!=null){if(dirCode==null){callback("ajax_directory_list.aspx?SelectedState="+selectedState,OnStateChange_DirectoryList);var dirName="- Select a Directory -";if(lang&&lang=='sp')
dirName="- Seleccione un libro -";SetDirectory("DUMMY",dirName,"");}else{callback("ajax_recent_directory.aspx?DirCode="+dirCode,OnStateChange_RecentDirectory);}
callback1("ajax_directory_dropdown.aspx?SelectedState="+selectedState+"&lang="+lang,OnStateChange_DirectoryDropDown);}else{SetDirectory("","","");var directoryCustomSelect=document.getElementById("directoryCustomSelect_dropdown");if(directoryCustomSelect){directoryCustomSelect.innerHTML="";}
